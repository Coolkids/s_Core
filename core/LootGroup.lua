...
if not (GetLocale=="enGB" or GetLocale=="enUS") then
	LOOT_ROLL_GREED = "%s 貪婪: %s"
	LOOT_ROLL_NEED = "%s 需求: %s"
	LOOT_ROLL_PASSED = "%s 放棄: %s"
	LOOT_ROLL_PASSED_AUTO = "%s 自動放棄: %s 因為他不能拾取這個物品."
	LOOT_ROLL_PASSED_AUTO_FEMALE = "%s 自動放棄: %s 因為他不能拾取這個物品."
	LOOT_ROLL_DISENCHANT = "%s 分解: %s"
end
...
function frame:UpdateGroupLoot()
	...
	for index, value in ipairs(grouplootlist) do
		...
		if ( not bar ) then
			...
			bar:EnableMouse(1);
			bar:SetWidth(DB.bar_width);
			bar:SetHeight(DB.bar_height);
			bar:SetStatusBarTexture(DB.bartex);
			if ( index == 1 ) then
			...
			bar.background:SetTexture(DB.bartex);
			...
			bar.border.topleft:SetTexture(DB.bordertex);
			...
			bar.border.bottomleft:SetTexture(DB.bordertex);
			...
			bar.border.topright:SetTexture(DB.bordertex);
			...
			bar.border.bottomright:SetTexture(DB.bordertex);
			...
			bar.border.top:SetTexture(DB.bordertex);
			...
			bar.border.bottom:SetTexture(DB.bordertex);
			...
			bar.border.left:SetTexture(DB.bordertex);
			...
			bar.border.right:SetTexture(DB.bordertex);
            ...
            bar.icon:SetSize(DB.roll_iconsize,DB.roll_iconsize)
            ...
            bar.iborder.topleft:SetTexture(DB.bordertex);
            ...
            bar.iborder.bottomleft:SetTexture(DB.bordertex);
            ...
            bar.iborder.topright:SetTexture(DB.bordertex);
            ...
            bar.iborder.bottomright:SetTexture(DB.bordertex);
            ...
            bar.iborder.top:SetTexture(DB.bordertex);
            ...
            bar.iborder.bottom:SetTexture(DB.bordertex);
            ...
            bar.iborder.left:SetTexture(DB.bordertex);
            ...
            bar.iborder.right:SetTexture(DB.bordertex);
		...
		bar.passtext:SetText(grouplootrolls[value.rollId] and grouplootrolls[value.rollId]["放棄"] and grouplootrolls[value.rollId]["放棄"].count or "");
		bar.disenchanttext:SetText(grouplootrolls[value.rollId] and grouplootrolls[value.rollId]["分解"] and grouplootrolls[value.rollId]["分解"].count or "");
		bar.greedtext:SetText(grouplootrolls[value.rollId] and grouplootrolls[value.rollId]["貪婪"] and grouplootrolls[value.rollId]["貪婪"].count or "");
		bar.needtext:SetText(grouplootrolls[value.rollId] and grouplootrolls[value.rollId]["需求"] and grouplootrolls[value.rollId]["需求"].count or "");
...
if not (GetLocale=="enGB" or GetLocale=="enUS") then
	...
	LOOT_ROLL_DISENCHANT = "%s 選擇分解: %s";
	LOOT_ROLL_DISENCHANT_SELF = "你選擇了分解: %s";
	LOOT_ROLL_GREED = "%s 選擇貪婪: %s";
	LOOT_ROLL_GREED_SELF = "你選擇貪婪: %s";
	LOOT_ROLL_NEED = "%s 選擇需求: %s";
	LOOT_ROLL_NEED_SELF = "你選擇需求: %s";
	LOOT_ROLL_PASSED = "%s 放棄了: %s";
	LOOT_ROLL_PASSED_AUTO = "%s 自動放棄了: %s 因為他不能拾取這件物品";
	LOOT_ROLL_PASSED_AUTO_FEMALE = "%s 自動放棄了: %s 因為他不能拾取這件物品.";
	LOOT_ROLL_PASSED_SELF = "你放棄了: %s";
	LOOT_ROLL_PASSED_SELF_AUTO = "你自動放棄了: %s 因為你不能拾取這件物品.";
	LOOT_ROLL_ROLLED_DE = "分解 - %d 在 %s 由 %s";
	LOOT_ROLL_ROLLED_GREED = "貪婪 - %d 在 %s 由 %s";
	LOOT_ROLL_ROLLED_NEED = "需求 - %d 在 %s 由 %s";
end
if DB.suppress_loot_spam then
	ChatFrame_AddMessageEventFilter("CHAT_MSG_LOOT", function(self, event, msg)
		if msg:match("(.*) has?v?e? selected (.+) for: (.+)") or msg:match("(.+) Roll . (%d+) for (.+) by (.+)")
			or msg:match("你放棄了: ") or msg:match("自動放棄了") or (msg:match(" 放棄了") and not msg:match("所有人都放棄了")) then
			return true
--[[ 			
